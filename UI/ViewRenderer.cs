namespace TheMine
{
    ...
    class ViewRenderer
    {
        ...
        public static void RenderTown(MapManager mapManager, Player player)
        {
            ...
            ConsoleRenderer.PrintTownOptions();
        ...
        public static void RenderShop(ShopManager shopManager, Player player, string currentInput)
        {
            ConsoleRenderer.BeginBuffer();
            ConsoleRenderer.DrawFrameToBuffer();
        ...
        public static void RenderEquipment(Player player, string currentInput)
        {
            ConsoleRenderer.BeginBuffer();
            ConsoleRenderer.DrawFrameToBuffer();
            ConsoleRenderer.ClearRegion(2, 2, 71, 22);
            ...
            for (int i = 0; i < equipments.Count; i++)
            {
                ...
                string numberStr = string.Format(" [{0:D2}]", i + 1);
                string nameStr = (equip.IsEquipped ? "[E]" : "") + equip.Name;
            ...
            int bottomY = ConsoleRenderer.GetBottomSplitY();
            ConsoleRenderer.BufferAt(3, bottomY + 1, "장착/해제할 장비의 [번호]를 입력하세요. (Q: 마을로 돌아가기)");
